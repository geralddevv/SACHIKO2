...
router.get("/tape/compare/:id", async (req, res) => {
  try {
    ...
    res.render("inventory/itemCompare.ejs", {
      ...
      orgLabel: "Fairtech",
      clientLabel: "Client",
      editBindingUrl: `/acme/tape-binding/edit/${binding._id}`,
...
router.post("/tape-binding/edit/:id", requireAuth, updateLimiter, async (req, res) => {
  try {
    ...
    if (typeof returnTo === "string" && returnTo.startsWith("/acme/")) {
      return res.redirect(returnTo);
    }

    res.redirect("/acme/tape/view/" + binding.userId);
  } catch (err) {
...
router.post("/tape-binding/delete/:id", requireAuth, deleteLimiter, async (req, res) => {
  try {
    ...
    return res.redirect(`/acme/tape/view/${binding.userId}`);
  } catch (err) {
