    ...
    items.forEach(item => {
      ...
      if (effectiveStock < minQty) {
        ...
        results.push({
          ...
          bindingPath: ({ Tape: "/acme/form/vendor-item-binding/tape" })[t.typeKey] || "/acme/vendor/coordinator/view"
        });
      }
    });
  }
...
router.get("/reorder/select-vendor-multi", async (req, res) => {
  try {
    ...
    if (!itemsParam) return res.redirect("/acme/inventory/orders/reorder");
    ...
    if (cartItems.length === 0) return res.redirect("/acme/inventory/orders/reorder");
...
router.post("/purchase/order", requireAuth, createLimiter, async (req, res) => {
  try {
    ...
    if (!binding) {
      ...
      return res.redirect("/acme/inventory/purchase/order");
    }
