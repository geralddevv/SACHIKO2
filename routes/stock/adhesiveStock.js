...
// Ceiling on one print run from the Print Drum Labels dialog. Well past any
...
router.post("/create", requireAuth, createLimiter, async (req, res) => {
  try {
    ...
    // The drums must add up to the Total Kg keyed off the invoice. Too
    ...
    const drumsKg = roundKg(drums.reduce((sum, r) => sum + (Number(r.reelMtrs) || 0), 0));
    const diff = roundKg(drumsKg - totalKg);
    if (Math.abs(diff) > 0.01) {
      const over = diff > 0;
      return res.status(400).json({
        success: false,
        message: `The ${drums.length} drum${drums.length === 1 ? "" : "s"} add up to ${drumsKg} kg, which is `
...
// The fields a drum is printed from, and its QR. Both the one-drum route and
// the many-drum sheet below go through this, so a label printed off a row's
// own Print button and the same drum printed as part of a batch are built
// from one place and cannot drift apart.
const LABEL_REEL_FIELDS = "rollId vendorName vendorSkuCode invoiceNo reelMtrs type gsm inwardDate";
...
router.get("/label/:stockId", requireAuth, async (req, res) => {
  try {
    ...
    const reel = await AdhesiveStock.findById(stockId).select(LABEL_REEL_FIELDS).lean();
...
// The same sticker, one per page, for however many drums were ticked in the
// stock page's Print Drum Labels dialog -- so a whole inward batch is stuck
// in one pass at the printer instead of a drum at a time.
//
// Only the ids travel: every value on every label is re-read here, exactly
// as the one-drum route reads it, so a doctored request can ask for labels
// for drums it shouldn't see (the same drums this page already lists) but
...
router.get("/labels", requireAuth, async (req, res) => {
  try {
    ...
    if (!ids.length) return sendLabelError(res, 400, "No drums were selected.");
    if (ids.length > MAX_LABELS_PER_SHEET) {
      return sendLabelError(res, 400, `Too many drums at once — ${MAX_LABELS_PER_SHEET} labels is the most one print can hold.`);
    }
    if (!ids.every((id) => mongoose.isValidObjectId(id))) return sendLabelError(res, 400, "One of the selected drums is not a valid drum.");

    const reels = await AdhesiveStock.find({ _id: { $in: ids } }).select(LABEL_REEL_FIELDS).lean();
    if (!reels.length) return sendLabelError(res, 404, "None of the selected drums could be found.");
    ...
    // as the drums on the screen, and can be matched off one by one.
    ...
    res.render("stock/adhesiveRollLabel.ejs", {
      ...
      mm: LABEL_MM,
      // A drum that has been deleted between opening the dialog and printing
      // is dropped rather than failing the whole sheet, but it is said out
      // loud on the page: a silently short stack is how a drum ends up with
      // no sticker on it.
      missingCount: ids.length - ordered.length,
    });
  } catch (err) {
    console.error("ADHESIVE ROLL LABEL SHEET ERROR:", err);
...
router.delete("/:id", requireAuth, deleteLimiter, async (req, res) => {
  try {
    const existing = await AdhesiveStock.findById(req.params.id);
    if (!existing) {
      return res.status(404).json({ success: false, message: "Adhesive stock drum not found." });
    }
