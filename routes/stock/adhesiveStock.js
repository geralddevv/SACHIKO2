...
router.post("/create", requireAuth, createLimiter, async (req, res) => {
  try {
    ...
    await AdhesiveStock.create({ ...payload, rollId });

    res.locals.auditDescription = `Added adhesive stock reel "${rollId}" (${payload.type}) at "${payload.location}"`;
    req.flash("notification", "Adhesive stock added successfully!");
...
router.put("/:id", requireAuth, updateLimiter, async (req, res) => {
  try {
    ...
    if (!updated) {
      return res.status(404).json({ success: false, message: "Adhesive stock reel not found." });
    }

    res.locals.auditDescription = `Updated adhesive stock reel "${updated.rollId}"`;
...
router.delete("/:id", requireAuth, deleteLimiter, async (req, res) => {
  try {
    ...
    res.locals.auditDescription = `Deleted adhesive stock reel "${existing.rollId}"`;
    res.json({ success: true });
  } catch (err) {
