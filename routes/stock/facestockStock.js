...
// Ceiling on one print run from the Print Reel Labels dialog. Well past any
...
const LABEL_REEL_FIELDS = "rollId vendorName vendorSkuCode invoiceNo reelMtrs size family type gsm micron inwardDate";
...
router.get("/label/:stockId", requireAuth, async (req, res) => {
  try {
    ...
    const reel = await FacestockStock.findById(stockId).select(LABEL_REEL_FIELDS).lean();
...
// stock page's Print Reel Labels dialog -- so a whole inward batch is stuck
...
router.get("/labels", requireAuth, async (req, res) => {
  try {
    ...
    const reels = await FacestockStock.find({ _id: { $in: ids } }).select(LABEL_REEL_FIELDS).lean();
    ...
    console.error("FACESTOCK ROLL LABEL SHEET ERROR:", err);
...
router.delete("/:id", requireAuth, deleteLimiter, async (req, res) => {
  try {
    const existing = await FacestockStock.findById(req.params.id);
    if (!existing) {
      return res.status(404).json({ success: false, message: "Facestock stock reel not found." });
    }

    // The Remove button already disables itself for a reel that's spoken
    // for, but that's only the page's view of it -- a reel can be allotted or
    // started on between the page load and the click. Re-read the same usage
    // the page was drawn from and refuse here, so removing a reel can never
    // leave an order's allotment or a job card's recorded usage pointing at
    // a reel that no longer exists.
    const { allottedByReel, usedByReel } = await loadFacestockReelUsage();
